function [diff_im, diff_total] = anisodiff2D(im, num_iter, delta_t, kappa, option)
...
%         diff_total(:,:,t) = diff_im;
        map = colormap(gray);
        figure(1), subplot 121, imshow(orig_im,[]), title('Original pic'), subplot 122, imshow(diff_im,[])
        f = getframe;
        diff_total(:,:,1,t) = rgb2ind(f.cdata,map,'nodither');
